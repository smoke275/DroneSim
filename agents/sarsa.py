...
class SARSAAgent(Agent):
    ...
    def _observation_to_state(self, observation):
        ...
        def direction_to(ugv, task):
            # 8 directions based on relative position
            dy = task[0] - ugv[0]
            dx = task[1] - ugv[1]
            if dy == 0 and dx == 0:
                return "STAY"
            # Determine general direction based on signs
            if dy > 0: # South component
                if dx > 0: return "SE"
                elif dx < 0: return "SW"
                else: return "S"
            elif dy < 0: # North component
                if dx > 0: return "NE"
                elif dx < 0: return "NW"
                else: return "N"
            else: # dy == 0, East or West
                if dx > 0: return "E"
                else: return "W" # dx must be < 0 here
        ...
        task_loads = observation['task_loads']
        directions = ["N", "NE", "E", "SE", "S", "SW", "W", "NW"]
        demand = {d: 0.0 for d in directions}
        total_load = 0.0
        for task, load in zip(task_coords, task_loads):
            dir = direction_to(ugv, task)
            if dir == "STAY":
                continue
            if dir in demand:
                demand[dir] += self.max_load + load
                total_load += self.max_load + load
        if total_load > 0:
            task_distribution = tuple(demand[d] / total_load for d in directions)
        else:
            task_distribution = tuple(0.0 for _ in directions)
        num_demand_bands = 5
        task_distribution = tuple(min(int(prob * num_demand_bands), num_demand_bands - 1) for prob in task_distribution)
        ...
        return (ugv, task_distribution, band, nb_traffic)
