...
def obs_to_tuple(obs):
    """
    Same observation-to-key encoding used in training.
    Must match the transformation that was used when building the Q-table.
    """
    tasks = tuple(obs['task_positions'].tolist())
    ugvs = tuple(obs['ugv_positions'].tolist())
    battery = tuple(map(int, obs['battery_levels']))
    return (tasks, ugvs, battery)

def choose_best_action(state, env, Q):
    """
    Returns the greedy (best) joint action under Q(s,a).
    We'll do a small partial search over each dimension
    to find the action that maximizes Q(s,a).
    """
    best_action = None
    best_q_val = float('-inf')

    # Because env.action_space is MultiDiscrete, 
    # we have an array specifying the size of each dimension.
    action_space_dims = env.action_space.nvec

    # We'll pick a few random candidates per dimension to keep it feasible
    subsets = []
    for dim_size in action_space_dims:
        # sample up to 3 distinct actions from each dimension
    ...
    for candidate_joint_action in itertools.product(*subsets):
        q_val = Q.get((state, candidate_joint_action), float('-inf'))
        if q_val > best_q_val:
            best_q_val = q_val
            best_action = candidate_joint_action

    # If no action found, return a random valid action
...
class Window(QMainWindow):
    ...
    def __init__(self):
        ...
        dp_agent = DPAgent(self.env, policy_name="66ad2a1e")
    ...
    def run(self):
        ...
        state = obs_to_tuple(observation)
        df_maze = info['maze']
        ...
        while not done:
            ...
            action = choose_best_action(state, self.env, self.q_table)
            # Step the simulation
